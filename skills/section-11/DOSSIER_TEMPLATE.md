...
This document serves as a reference template for endurance athletes using the deterministic
AI-coaching framework defined in Section 11.
...
**Adjustment Control:** Weight adjustments only permitted during readiness-positive periods
(DI ≥ 0.95, HR drift ≤ 3%, RI ≥ 0.8)
...
> **Race tagging for automated protocol activation:** Tag races in Intervals.icu as `RACE_A`,
> `RACE_B`, or `RACE_C` using the event category selector. The race-week protocol (Section 11A)
> activates automatically for A and B races within 7 days. C races are training races — no taper
> adjustments. For best results, also set expected duration (`moving_time`) in the event to
> enable event-type modifiers (carb loading, opener intensity, TSB targets).
...
**Repo:** `[username]/[repo]` (connected via platform's GitHub integration — AI reads files
directly, no URLs needed)

> **Tip:** If you commit `DOSSIER.md` to your data repo alongside `latest.json`, `history.json`,
> and `intervals.json`, connecting the repo gives the AI both your data and your profile in one
> connection. The only remaining piece is `SECTION_11.md`, which is bundled in this skill.

**— OR (Google Drive connector) —**

**Folder:** `training-data/` in Google Drive (connected via Google Drive integration)

**Files:** `latest.json`, `history.json`, `intervals.json`, `routes.json`, `DOSSIER.md`

> The Drive connector works on Claude mobile too — once authorized on desktop, available
> everywhere. Combine with the Intervals.icu → Drive sync (see json-local-sync example) for
...
**Intervals:** `intervals.json` (data directory root)

---

## Protocol Reference

This dossier follows the **Section 11 AI Coach Guidance Protocol**.

**Protocol Location:** Bundled in this skill as `SECTION_11.md`
